...
screen choice(items):
    ...
    vbox:
        ...
        for i in items:
            frame:
                add None blur 10
                # background Frame(Transform("gui/frame.png", alpha=1.0), 15, 15)
                background "#081820de"
                xfill True
                ysize 80
...
screen navigation():
    ...
    hbox:
        ...
        if main_menu:
            ...
            textbutton _("开始游戏") text_size 48 action Start()

        else:

            textbutton _("历史") text_size 48 action ShowMenu("history")

            textbutton _("保存") text_size 48 action ShowMenu("save")

        textbutton _("读取游戏") text_size 48 action ShowMenu("load")

        textbutton _("设置") text_size 48 action ShowMenu("preferences")

        if _in_replay:

            textbutton _("结束回放") text_size 48 action EndReplay(confirm=True)

        elif not main_menu:

            textbutton _("标题菜单") text_size 48 action MainMenu()

        textbutton _("关于") text_size 48 action ShowMenu("about")

        if renpy.variant("pc") or (renpy.variant("web") and not renpy.variant("mobile")):

            ## “帮助”对移动设备来说并非必需或相关。
            textbutton _("帮助") text_size 48 action ShowMenu("help")

        if renpy.variant("pc"):

            ## 退出按钮在 iOS 上是被禁止使用的，在安卓和网页上也不是必要的。
            textbutton _("退出") text_size 48 action Quit(confirm=not main_menu)
...
transform hover_button:
    on idle:
        linear 1.0 yanchor 0
    on hover:
        easein 1.5 yanchor 5
        block:
            easein 1.5 yanchor -5
            easein 1.5 yanchor 5
            repeat
...
screen main_menu():
    ...
    frame background "#081820":
        ...
        button:
            ...
            xpos 144 * 8
            ypos 54 * 8
            text "开始新游戏" yalign 0.5 xalign 1.0
            action Start()
        ...
        button:
            ...
            xpos 138 * 8
            ypos 67 * 8
            text "继续游戏" yalign 0.5 xalign 1.0
